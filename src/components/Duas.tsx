...
const categoryIcons: Record<string, string> = {
  'الصلاة': '🕌', 'الكرب والضيق': '💙', 'الرزق': '✨', 'المرض': '💊',
  'السفر': '✈️', 'النوم': '🌙', 'الطعام': '🍽️', 'الاستخارة': '🤲',
  'القرآن': '📖', 'الوالدين': '❤️',
};
  ...
  return (
    <div className="h-full overflow-y-auto bg-gradient-to-b from-purple-50 to-violet-50">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          <button onClick={() => setSelectedCategory(null)}
            className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition-all border ${
              !selectedCategory ? 'bg-purple-600 text-white border-purple-600' : 'bg-white text-gray-600 border-gray-200 hover:border-purple-300'
            }`}>
          ...
          {duaCategories.map(cat => (
            <button key={cat} onClick={() => setSelectedCategory(selectedCategory === cat ? null : cat)}
              className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-full text-sm font-semibold transition-all border ${
                selectedCategory === cat ? 'bg-purple-600 text-white border-purple-600' : 'bg-white border-gray-200 hover:border-purple-300 text-gray-600'
              }`}>
      ...
      <div className="p-4 space-y-4 max-w-2xl mx-auto">
        ...
        {filtered.map(dua => (
          <div key={dua.id} className="bg-white rounded-2xl shadow-sm overflow-hidden card-hover">
            <div className="flex items-center justify-between px-4 pt-4 pb-2">
              <span className={`text-xs font-bold px-3 py-1 rounded-full border ${categoryColors[dua.category] || 'bg-gray-100 text-gray-600 border-gray-200'}`}>
                {categoryIcons[dua.category]} {dua.category}
              </span>
              <h3 className="font-bold text-gray-800">{dua.title}</h3>
            </div>
            <div className="px-4 pb-3">
              <p className="arabic-text text-gray-800 text-right text-lg leading-loose">{dua.arabic}</p>
            </div>
            <div className="px-4 pb-3">
              <p className="text-gray-500 text-sm text-right leading-relaxed border-r-2 border-purple-200 pr-3">{dua.translation}</p>
            </div>
            <div className="border-t border-gray-100 px-4 py-3 flex items-center justify-between bg-gray-50">
              <div className="flex items-center gap-2">
                <button onClick={() => toggleFavorite(dua.id)}
                  className={`p-1.5 rounded-lg transition-colors ${favorites.has(dua.id) ? 'text-red-500 bg-red-50' : 'text-gray-400 hover:text-red-400 hover:bg-red-50'}`}>
                  <Heart size={16} fill={favorites.has(dua.id) ? 'currentColor' : 'none'} />
                </button>
                <button onClick={() => copyDua(dua.arabic, dua.id)} className="p-1.5 rounded-lg text-gray-400 hover:text-purple-500 hover:bg-purple-50 transition-colors">
                  {copiedId === dua.id ? <Check size={16} className="text-emerald-500" /> : <Copy size={16} />}
                </button>
                <button onClick={() => navigator.share?.({ text: `${dua.title}\n\n${dua.arabic}` })} className="p-1.5 rounded-lg text-gray-400 hover:text-purple-500 hover:bg-purple-50 transition-colors">
                  <Share2 size={16} />
                </button>
              </div>
        ...
        {favorites.size > 0 && (
          <div className="text-center text-xs text-gray-400 py-2">❤️ {favorites.size} دعاء محفوظ في المفضلة</div>
        )}
      </div>
    </div>
  );

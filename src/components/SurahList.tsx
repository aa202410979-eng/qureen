          ...
          {(['all', 'meccan', 'medinan'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`flex-1 text-xs py-1 rounded-full transition-colors ${
                filter === f ? 'bg-emerald-500 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
              }`}>
            ...
            <button key={surah.number} onClick={() => onSelectSurah(surah)}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-emerald-600">{surah.revelationType === 'Meccan' ? 'مكية' : 'مدنية'}</span>
                </div>
              </div>
            </button>
          ))
